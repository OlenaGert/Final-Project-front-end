...
const eventsStore = [
  ...
  events.forEach(event => {
    ...
    const distanceInfo = event.type === "offline" ? ` (${event.distance} km)` : "";
    const typeLabel = event.type === "online" ? `<span class="event-type">Online Event</span>` : "";
    ...
    const cardHTML = `
      <div class="event-item">
        ...
        <div class="event-info">
          <p>${date} UTC</p>
          <h3>${event.title}</h3>
          <p>${event.category}${distanceInfo}</p>
  ...
  const filteredEvents = eventsStore.filter(event => {
    ...
    const distanceMatch = !selectedDistance || event.distance <= parseInt(selectedDistance);
